    ...
    if($_POST)
    {
        ...
        if (!$conn) {
            ...
            try
            {
                ...
                $plsql = "BEGIN PaqueteGlobal.AcogerAnimal('$Id', '$IdAnimal', SYSDATE); END;";  
                
                $stmt = oci_parse($conn, $plsql);

                $flag2 = oci_execute($stmt);

                $resultado = "<div id=\"Resultado\"><p> La acogida ha sido exitosa, puede recoger al animal en nuestras oficinas en un plazo de 24 horas. </p></div>";
                echo $resultado;
            }
