...
<body>
    ...
    if($_POST)
    {
        ...
        if (!$conn) {
            ...
            try
            {
                $plsql = "BEGIN PaqueteGlobal.DarDeAltaPersona('$DNI', '$Nombre', '$Apellidos', '$Telefono', '$Correo'); END;";
                ...
                if(!$flag1)
                {
                    throw new OCIException('Estos datos ya se encuentran registrados en el sistema',500);
                }
                
                $sql = "SELECT IdPersona FROM TablaPersona WHERE DNI = '$DNI'";
                $stmt = oci_parse($conn, $sql);
    
                $resultado =  "<div id=\"Resultado\"> <p> Registro correcto, su ID es: ";
                ...
                while ($row = oci_fetch_array($stmt, OCI_ASSOC+OCI_RETURN_NULLS))
                {
                    foreach ($row as $item)
                    {
                        if (is_object($item) && get_class($item) === 'OCILob') {
                            ...
                            $data = $item->load();
                            $resultado = $resultado . $data . "</p></div>";
                        } else {
                            $resultado = $resultado . $item . "</p></div>";
                        }
                    }
                    echo $resultado;
                }
            ...
            {
                $resultado = "<div id=\"Resultado\"> <p> Registro incorrecto, los datos ya se encuentran registrados en el sistema </p></div>";
                echo $resultado;
            }
